...
export class ContactService {
  ...
  private async searchRegularContacts(query: string, maxResults?: number): Promise<Contact[]> {
    try {
      ...
      return this.filterAndConvertContacts(response.data.connections as GoogleContact[], query, 'contacts');
    } catch (error) {
  ...
  private async searchOtherContacts(query: string, maxResults?: number): Promise<Contact[]> {
    try {
      ...
      return this.filterAndConvertContacts(response.data.otherContacts as GoogleContact[], query, 'other_contacts');
    } catch (error) {
