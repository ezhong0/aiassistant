/**
 * Simple test for email agent JSON parsing fix
 */
...
function testJsonParsing() {
  ...
  // Test the private method indirectly by creating a mock OpenAI response
  const testResponses = [
  ...
  testResponses.forEach((response, index) => {
    ...
    try {
      ...
      console.log(`❌ Parse failed (this is expected for invalid JSON): ${parseError instanceof Error ? parseError.message : String(parseError)}`);
    }
    console.log('');
  });

  console.log('🎉 JSON parsing test completed!');
  console.log('The fix should handle markdown code blocks and extract JSON from mixed content.');
}
